$(function(){
    ...
    for(var i=1; i<8;i++){
		...
		// Hide the labels
        $("label[for='time_preference_" + i +"']").css("visibility","hidden");

		$("#time_preference_" + i + " input[type='checkbox']").each(function() {
	...
	$("#select_all_day").click(function(){
		...
		/*for(var i=1; i<8;i++){
			$("#time_preference_"+ i).css("visibility","visible");
		}*/
    });
	...
	$("#select_all_time").click(function(){
        //$('#day_preferences input').prop('checked', 'checked');
        //$('.time_pref input').prop('checked', 'checked');

        if(cntrlr.all_days_selected){
			$('#day_preferences input').prop('checked', false);
        	$('.time_pref input').prop('checked', false);
			cntrlr.all_days_selected = false;
		}else{
			$('#day_preferences input').prop('checked', 'checked');
        	$('.time_pref input').prop('checked', 'checked');
	...
	$("form").submit(function(){
        ...
        $('#day_preferences input[type = "checkbox"]:checked ').each(function(){
            ...
            var id = "time_preference_" + checked_field;
        ...
        if(arr.length > 0)
        {
            alert("Please select time preference for " + arr.join(", "));
			failed = true;
...
function activate_time_checkbox(obj) {
	...
	// Get selected checkbox value
        ...
        var id_time = "time_preference_"+ id ;
        ...
        if(obj.is(":checked")){
			...
			$(".time_checkbox_" + id).change(function() {
				...
				if (checked_counter == class_length) {
					console.log('ehere');
