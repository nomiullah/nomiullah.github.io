$(function () {
	...
	$('.mobile-filter').click(function(){
		if( $(this).hasClass('active')){
			...
			$('.sidebar-pet-search').stop(true,true).animate({'height':'40px'});
		}
		else{
			$(this).addClass('active');
	...
	// floating section for pet-search
	$(window).scroll(function () {
		var floatView = $('#wrap-in').height();
		var floatWrapperView = $('#float-conttainer-wrap').height();
		var headerView = $('#header-wrap').outerHeight();
		var footerView = $('#footer-wrap').outerHeight();
		var gapView = $('.top-section').outerHeight();
		var docView = $(document).height();
		var windowView = $(window).height();
		
		if ($(this).scrollTop()>=headerView+gapView){
			$('#wrap-in').css('position', 'absolute').animate({ top: $(window).scrollTop()-headerView-gapView+'px'}, 0);
			$('#wrap-in').css('right', '19px');
			
			if($(this).scrollTop()-headerView-gapView>floatWrapperView-floatView){
